...
class ShortLimitholdemRuleAgentV1(object):
    ''' Short Limit Hold 'em Rule agent version 1
    '''
...
class ShortLimitholdemRuleAgentV2(object):
    ''' Short Limit Hold 'em Rule agent version 2
    '''
    ...
    def step(state):
        ...
        if len(public_cards) == 3:
            ...
            for i, _ in enumerate(public_cards):
                public_cards_ranks[i] = public_cards[i][1]
                public_cards_flush[i] = public_cards[i][0]
            #if there is still a pair
            if hand[0][1] in public_cards_ranks or hand[1][1] in public_cards_ranks:
                    action = 'raise'
            elif hand[0][1] == 'A' or hand[1][1] == 'A':
                if 'K' in [hand[0][1], hand[1][1]] or 'Q' in [hand[0][1], hand[1][1]] or 'J' in [hand[0][1], hand[1][1]] or 'T' in [hand[0][1], hand[1][1]]:
                    # For AK, AQ, AJ, AT types, if public cards have A, K, Q, J, T, raise, because the chance of getting a straight greatly increases
...
class ShortLimitholdemRuleModel(Model):
    ...
    def __init__(self):
        ...
        rule_agent = ShortLimitholdemRuleAgentV1()
